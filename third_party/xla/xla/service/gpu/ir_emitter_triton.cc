...
absl::Status EmitMatMul(mlir::OpBuilder builder,
  ...
  HloInstructionAdaptor instrAdaptor{*instr};
  ...
  bool is_8_bit_or_less_dot_with_F32 = HloAnyOf(
      instrAdaptor.GetOperands(), *fusion, [&](HloInstructionAdaptor node) {
        if (node.opcode() == HloOpcode::kConvert) {
          Type inType =
              TritonType(builder, node.GetOperand(0).shape().element_type());
          Type outType = TritonType(builder, node.shape().element_type());
          if (inType.getIntOrFloatBitWidth() <= 8 && outType.isF32()) {
            return true;
          }
        }
        return false;
